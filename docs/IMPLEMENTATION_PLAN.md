...
| D6 | Auth | **Bearer-Token + Identität via Header** | Shared Secret zwischen OWUI und HiveMCP. Nutzeridentität über OWUIs Header-Templates (`{{USER_ID}}`, `{{USER_EMAIL}}`, `{{USER_GROUPS}}`) — reicht für Template-Sichtbarkeit, Quotas und Audit ohne eigenes IdP. |
| D7 | GUI-Flow | **Formular → `input:prompt:submit`** | Das iframe schickt die gewählten Parameter per `postMessage` als Folge-Prompt in den Chat; das Modell ruft daraufhin `hive_create_*` auf. Funktioniert **ohne** `allowSameOrigin` (nur ein Bestätigungsdialog) — wichtig, weil dieser Schalter pro Nutzer standardmäßig aus ist. |
